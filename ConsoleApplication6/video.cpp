...
float pointNorm(Point2f p1, Point2f p2) {
	return sqrt(pow((p1.x - p2.x), 2) + pow((p1.y - p2.y), 2));
}
float cornerDistance(float x1, float x2, float y1, float y2, float d) {
	
	float e = -100;
	float result = pow((x1 / (d + e) - x2 / (d - e)), 2) + pow((y1 / (d + e) + y2 / (d - e)), 2);
	return e;
	while (abs(result-100) > 5) {
		//cout << endl << e;
		//cout << endl << abs(result);
		if (e == 0) {
			e+=.1;
		}
		
		result = pow((x1 / (d + e) - x2 / (d - e)), 2) + pow((y1 / (d + e) + y2 / (d - e)), 2);
		if (e > 100) {
			cout << "nonononno";
			cin.ignore();
		}
		...
		if (mark >= 2)		// Ensure we have (atleast 3; namely A,B,C) 'Alignment Markers' discovered
		{
			...
			if (top < contours.size() && right < contours.size() && bottom < contours.size() && contourArea(contours[top]) > 10 && contourArea(contours[right]) > 10 && contourArea(contours[bottom]) > 10)
			{
				...
				if (orientation == CV_QR_NORTH) {
					...
					BR.x = BL.x + TR.x - TL.x;
					BR.y = BL.y + TR.y - TL.y;
					offset = cvPoint(BR.x - TR.x, BR.y - TR.y);
				}

				if (orientation == CV_QR_EAST) {
					BR = mc[A];
					TR = mc[B];
					TL = mc[C];
					...
					BL.x = TL.x - TR.x + BR.x;
					BL.y = BR.y - TR.y + TL.y;
					offset = cvPoint(BL.x - BR.x, BL.y - BR.y);
				}

				if (orientation == CV_QR_SOUTH) {
					TL = mc[A];
					BR = mc[B];
					TR = mc[C];
					...
					TL.x = BL.x + TR.x - BR.x;
					TL.y = BL.y + TR.y - BR.y;
					offset = cvPoint(TL.x - BL.x, TL.y - BL.y);
				}
				if (orientation == CV_QR_WEST) {
					BR = mc[A];
					BL = mc[B];
					TL = mc[C];
					...
					TR.x = TL.x + BR.x - BL.x;
					TR.y = TL.y + BR.y - BL.y;
					offset = cvPoint(TR.x - TL.x, TR.y - TL.y);
				}

				float topdiff = pointNorm(TL, TR);
				float rightdiff = pointNorm(BR, TR);
				float bottomdiff = pointNorm(BR, BL);
				float leftdiff = pointNorm(TL, BL);
				float cdiff1 = pointNorm(BR, TL);
				float cdiff2 = pointNorm(TR, BL);

				//cout << endl << "diffs" << topdiff << rightdiff << bottomdiff << leftdiff <<cdiff1 << cdiff2;
				if (topdiff < 40 | rightdiff < 40 | bottomdiff < 40 | leftdiff < 40 | cdiff1 < 50 | cdiff2 < 50) {
					continue;
				}
				...
				float temppoints[8] = { TL.x, TL.y,
									TR.x, TR.y, 
									BR.x, BR.y,
									BL.x, BL.y };

				float tempstd[8] = { 100, 100,
					200, 100,
					200, 200,
					100, 200};

				Mat points(4, 2, CV_32F, temppoints);

				Mat stdpoints(4, 2, CV_32F, tempstd);
				
				Mat origH = findHomography(points, stdpoints);

				origH.convertTo(H, CV_32F);
				...
				Mat topleft = (Mat_<float>(3, 1) << TL.x, TL.y, 1);
				Mat topright = (Mat_<float>(3, 1) << TR.x, TR.y, 1);
				Mat bottomright = (Mat_<float>(3, 1) << BR.x, BR.y,  1);
				Mat bottomleft = (Mat_<float>(3, 1) << BL.x, BL.y, 1);
				Mat invH = origH.inv();
				invH.convertTo(invH, CV_32F);
				//cout << endl << H << endl;
				//H.at<float>(8) = 1;
				
				//Mat topleft = (Mat_<float>(3, 1) << 100, 100, 1);
				//Mat topright = (Mat_<float>(3, 1) << 200, 100, 1);
				//Mat bottomright = (Mat_<float>(3, 1) << 200, 200, 1);
				//Mat bottomleft = (Mat_<float>(3, 1) << 100, 200, 1);

				Mat ttopleft = H * topleft;
				Mat ttopright = H * topright;
				Mat tbottomright = H * bottomright;
				Mat tbottomleft = H * bottomleft;
				cout << endl << ttopleft.at<float>(0);
				CvPoint tlcenter = cvPoint(ttopleft.at<float>(0)/ttopleft.at<float>(2), ttopleft.at<float>(1) / ttopleft.at<float>(2));
				CvPoint trcenter = cvPoint(ttopright.at<float>(0) / ttopright.at<float>(2), ttopright.at<float>(1) / ttopright.at<float>(2));
				CvPoint brcenter = cvPoint(tbottomright.at<float>(0) / tbottomright.at<float>(2), tbottomright.at<float>(1) / tbottomright.at<float>(2));
				CvPoint blcenter = cvPoint(tbottomleft.at<float>(0) / tbottomleft.at<float>(2), tbottomleft.at<float>(1) / tbottomleft.at<float>(2));
				
				Mat topleft2 = (Mat_<float>(3, 1) << 200, 200, 1);
				Mat topright2 = (Mat_<float>(3, 1) << 400, 200, 1);
				Mat bottomright2 = (Mat_<float>(3, 1) << 400, 400, 1);
				Mat bottomleft2 = (Mat_<float>(3, 1) << 200, 400, 1);



				Mat ttopleft2 = invH * topleft2;
				Mat ttopright2 = invH * topright2;
				Mat tbottomright2 = invH * bottomright2;
				Mat tbottomleft2 = invH * bottomleft2;

				//cout << tbottomright2;

				CvPoint tlcenter2 = cvPoint(ttopleft2.at<float>(0) / ttopleft2.at<float>(2), ttopleft2.at<float>(1) / ttopleft2.at<float>(2));
				CvPoint trcenter2 = cvPoint(ttopright2.at<float>(0) / ttopright2.at<float>(2), ttopright2.at<float>(1) / ttopright2.at<float>(2));
				CvPoint brcenter2 = cvPoint(tbottomright2.at<float>(0) / tbottomright2.at<float>(2), tbottomright2.at<float>(1) / tbottomright2.at<float>(2));
				CvPoint blcenter2 = cvPoint(tbottomleft2.at<float>(0) / tbottomleft2.at<float>(2), tbottomleft2.at<float>(1) / tbottomleft2.at<float>(2));

				float xdiff1 = tlcenter.x - blcenter.x;
				float ydiff1 = tlcenter.y - blcenter.y;
				float xdiff2 = tlcenter2.x - blcenter2.x;
				float ydiff2 = tlcenter2.y - blcenter2.y;

				float mag1 = sqrt(xdiff1 * xdiff1 + ydiff1 * ydiff1);
				float mag2 = sqrt(xdiff2 * xdiff2 + ydiff2 * ydiff2);

				float distance = mag1/mag2;
				//distance = (tlcenter.y - blcenter.y) / 100.0;
				cout << "distance " << distance << "wat " << endl; // This is the z distance

				distance = distance * 100;

				float tld,  trd,  brd,bld;

				tld = cornerDistance(TL.x, TR.x, TL.y, TR.y, distance);
				tld += cornerDistance(TL.x, BL.x, TL.y, BL.y, distance);

				brd = cornerDistance(BR.x, BL.x, BR.y, BL.y, distance);
				brd += cornerDistance(BR.x, TR.x, BR.y, TR.y, distance);

				trd = cornerDistance(TR.x, TL.x, TR.y, TL.y, distance);
				trd += cornerDistance(TR.x, BR.x, TR.y, BR.y, distance);

				bld = cornerDistance(BL.x, BR.x, BL.y, BR.y, distance);
				bld += cornerDistance(BL.x, TL.x, BL.y, TL.y, distance);
				...
				Mat tl3D = (Mat_<float>(3, 1) << TL.x/tld, TL.y/tld, tld);
				Mat tr3D = (Mat_<float>(3, 1) << TR.x/trd, TR.y/trd, trd);
				Mat br3D = (Mat_<float>(3, 1) << BR.x/brd, BR.y/brd, brd);
				Mat bl3D = (Mat_<float>(3, 1) << BL.x/bld, BL.y/bld, bld);
				...
				//Mat direction;
				//Mat topvec = (Mat_<float>(3, 1) << TL.x - TR.x, TR.y - BR.y, distance);
				//Mat rightvec = TR - BR;


				Mat stdcenter = (Mat_<float>(3, 1) << 150, 150, 1);
				Mat center = invH * stdcenter;
				center = center / center.at<float>(2);
				CvPoint centerpoint = cvPoint(center.at<float>(0), center.at<float>(1));
				circle(image, centerpoint, 10, Scalar(255, 0, 0));

				float x3D = centerpoint.x / distance;
				float y3D = centerpoint.y / distance;
				cout << endl << "Center: " << x3D << " " << y3D;


				drawContours(image, contours, top, Scalar(255, 0, 0), 2, 8, hierarchy, 0);
				drawContours(image, contours, right, Scalar(0, 255, 0), 2, 8, hierarchy, 0);
				drawContours(image, contours, bottom, Scalar(0, 0, 255), 2, 8, hierarchy, 0);
				drawContours(image, contours, right, Scalar(0, 255, 255), 2, 8, hierarchy, 0, offset);
